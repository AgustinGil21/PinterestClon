...
const BoardCollage = ({ className, collage = [] }: BoardCollageProps) => {
  return (
    ...
    >
      <div
        className={`w-full row-[span_2] col-[1] bg-[#e9e9e9] rounded-l-lg  bg-center bg-cover bg-no-repeat`}
        style={{ backgroundImage: `url(${collage[0] || ''}) ` }}
      ></div>
      <div
        className={`w-full row-[1] col-[2] bg-[#e9e9e9] rounded-tr-lg bg-center bg-cover bg-no-repeat`}
        style={{ backgroundImage: `url(${collage[1] || ''})` }}
      ></div>
      <div
        className={`w-full row-[2] col-[2] bg-[#e9e9e9] rounded-br-lg bg-center bg-cover bg-no-repeat`}
