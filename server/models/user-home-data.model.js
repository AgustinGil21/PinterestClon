...
export default class UserHomeDataModel {
  static async getData({ id }) {
    const response = await pool.query(
      'SELECT username, email_address, users.name, surname, verified, avatar, avatar_background, avatar_letter_color, avatar_letter ,account_types.name AS account_type, l.abbreviation AS lang FROM users INNER JOIN account_types ON account_types.id = account_type_id LEFT JOIN languages l ON l.id = lang_id WHERE users.id = $1;',
      [id]
    );
