...
const areValuesDifferent = (val1, val2) => {
  ...
  if (Array.isArray(val1) && Array.isArray(val2)) {
    // Comparar arrays
    if (val1.length !== val2.length) return true; // Diferentes longitudes
    for (let i = 0; i < val1.length; i++) {
      if (areValuesDifferent(val1[i], val2[i])) return true; // Comparar elementos
    }
    return false;
  }
  ...
  if (typeof val1 === 'object' && typeof val2 === 'object') {
    ...
    if (keys1.length !== keys2.length) return true; // Diferente número de claves

    for (let key of keys1) {
      if (areValuesDifferent(val1[key], val2[key])) return true; // Comparar valores de las claves
    }
    return false;
  }
